...
export const createBlog = async (req, res) => {
  try {
    ...
    const imageValue = typeof image === 'string' ? image : (req.file ? `/uploads/${req.file.filename}` : '');
    const created = await Blog.create({ author, title, image: imageValue, excerpt });
    res.status(201).json(created);
  } catch (err) {
    const status = err?.message?.includes('image') ? 400 : 500;
    res.status(status).json({ message: err?.message || 'Failed to create blog' });
  }
};
